...
type ChainInfo struct {
	...
	AcrossDepositContractAddress sql.NullString
...
func (mgr *ChainInfoManager) LoadAllChains() {
	...
	for rows.Next() {
		...
		if err = rows.Scan(&chain.Id, &chain.ChainId, &chain.RealChainId, &chain.Name, &chain.AliasName, &chain.Backend,
			...
			&chain.GasTokenDecimal, &chain.GasTokenIcon, &chain.TransferContractAddress, &chain.DepositContractAddress, &chain.AcrossDepositContractAddress,
			...
			chain.AcrossDepositContractAddress.String = strings.TrimSpace(chain.AcrossDepositContractAddress.String)
